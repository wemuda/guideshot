...
import { Button, buttonVariants } from '@guideshot/ui/components/button'
...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      className={cn(
        'group/calendar bg-surface p-3 text-cell [--cell-radius:var(--radius-control)] [--cell-size:32px] in-data-[slot=card-content]:bg-transparent in-data-[slot=popover-content]:bg-transparent',
      ...
      classNames={{
        ...
        caption_label: cn(
          ...
          captionLayout === 'label'
            ? 'text-body'
            : 'flex items-center gap-1 rounded-(--cell-radius) text-body [&>svg]:size-3.5 [&>svg]:text-text-meta',
          defaultClassNames.caption_label
        ),
        month_grid: cn('w-full border-collapse', defaultClassNames.month_grid),
        weekdays: cn('flex', defaultClassNames.weekdays),
        weekday: cn(
          'flex-1 rounded-(--cell-radius) text-caption font-medium text-text-meta select-none',
      ...
      components={{
        ...
        Chevron: ({ className, orientation }) => {
          if (orientation === 'left') {
            return <Icon icon={ArrowLeftIcon} size={16} className={className} />
          }

          if (orientation === 'right') {
            return (
              <Icon icon={ArrowRightIcon} size={16} className={className} />
            )
          }

          return <Icon icon={ArrowDownIcon} size={16} className={className} />
        },
        DayButton: ({ ...props }) => (
